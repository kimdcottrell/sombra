...
nums = DataDump::BARCODE_MASH.each do |l|
  l.split("-").each_with_index do |c,i|
    ...
    # 1. use to_i(16) to convert our barcode mash sections (e.g. FEC), which are Base16 strings, to integer
    # 2. use to_s(2) to convert our Base16 int into Base2 (binary) string
    # 3. assume that 12 or 13 is the key since there were 12 or 13 sections always, so pad our string 13 spaces over
